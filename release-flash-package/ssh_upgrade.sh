...
usage() {
    cat <<EOF
Usage:
  $SCRIPT_NAME --host root@192.168.1.1
...
Options:
  ...
  --boot FILE             boot.img to flash to the boot partition. Default:
                           ./boot.img in the current working directory.
  --rootfs FILE           gzip-compressed raw rootfs image. Default:
                           ./rootfs.raw.img.gz when present.
  --system FILE           Android sparse or raw system.img. Sparse images are
                           converted locally to rootfs.raw.img.gz with simg2img.
                           Default: ./system.img when --rootfs is not used and
                           ./rootfs.raw.img.gz is not present.
...
if [ -z "$BOOT_IMG" ] && [ -f "$DEFAULT_IMAGE_DIR/boot.img" ]; then
    BOOT_IMG="$DEFAULT_IMAGE_DIR/boot.img"
fi
if [ -z "$ROOTFS_GZ" ] && [ -z "$SYSTEM_IMG" ]; then
    if [ -f "$DEFAULT_IMAGE_DIR/rootfs.raw.img.gz" ]; then
        ROOTFS_GZ="$DEFAULT_IMAGE_DIR/rootfs.raw.img.gz"
    elif [ -f "$DEFAULT_IMAGE_DIR/system.img" ]; then
        SYSTEM_IMG="$DEFAULT_IMAGE_DIR/system.img"
    fi
fi
