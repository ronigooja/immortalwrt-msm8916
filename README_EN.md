...
1. First, fork this repository to your own account
   ![Fork repo](assets/screenshots/0.png)

2. Your repo → **Actions** → **Build ImmortalWrt MSM8916** → **Run workflow** → Select your device model → Enter required plugin packages → Run workflow to start building
   ![Build tutorial](assets/screenshots/1.png)

3. Build takes approx. ⏱️ 1.5–2 hours; more plugins = longer build time
   ![Build process](assets/screenshots/2.png)

4. A green checkmark means a successful build. Your repo → **Releases** → Download the firmware package
   ![Download firmware](assets/screenshots/3.png)

5. If your device is already running Linux or OpenWrt, follow the steps below to upgrade.
   ![Upgrade firmware](assets/screenshots/4.png)
   ...
   ![Flash OpenWrt from Android](assets/screenshots/5.png)
   ...
   ![Restore firmware](assets/screenshots/6.png)
...
| `assets/screenshots/`  | Tutorial screenshots used in the README                                                       |
| `scripts/`             | Helper scripts executed during the build                                                      |
| `legacy-tools/`        | Archived flashing tools, 9008 drivers, baseband bundles, and other historical helper assets   |
| `release-flash-package/` | Files integrated into the one-click flash package after a successful build                  |
